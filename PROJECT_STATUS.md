...
5. **일목균형표 상단 공간 게이트 (Overhead Space Gate):**
   - 구름대 아래 역배열 바닥 반등 종목은 1차 저항선($\min(CloudBottom, MA_{60})$)까지 최소 $+5.0\%$ 이상의 잔여 공간이 없으면 진입 즉시 차단(Veto).
6. **2단계 포지션 라이프사이클 & Free-Ride 시스템:**
   - 1차 목표가(TP1, $+20\%$): 물량 50% 분할 익절 확정.
   - 무위험 본전보호 전환: 잔여 50% 손절선을 본전가($Entry \times 1.005$)로 상향하여 원금 100% 무위험 보호(Free-Ride).
   - 동적 기한 연장: 빅스윙 추세 추종을 위해 최대 보유 기한을 20영업일 ➔ 40영업일로 연장하여 2차 목표가(TP2, $+50\%$) 조준.
   - 긴급 알림: TP1 도달 시 실시간 텔레그램 분할익절 & 기한연장 알림 자동 발송.
